...
# Safety check – catches stale views immediately
expected_cols = [
    "person", "category", "order_year", "order_quarter",
    "total_orders", "total_sales", "total_profit"
]
missing = [c for c in expected_cols if c not in df.columns]
if missing:
    st.error(f"❌ Missing columns in view: {missing}")
    st.info("Did you run `python transform.py` after updating the SQL?")
    st.write("**Columns currently in view:**", df.columns.tolist())
    st.stop()
